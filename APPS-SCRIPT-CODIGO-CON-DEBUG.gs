...
function doPost(e) {
  try {
    ...
    Logger.log("[DEBUG] Action recibida: " + (data.action || "NO ACTION"));
    Logger.log("[DEBUG] Data completa: " + JSON.stringify(data));
    
    // Si es acción de crear carpeta
    if (data.action === "createFolder") {
      Logger.log("[DEBUG] Ejecutando createFolder_");
    ...
    if (data.action === "getOrCreateFolder") {
      Logger.log("[DEBUG] Ejecutando getOrCreateFolder_");
      return getOrCreateFolder_(data);
    }
    
    // Si no tiene action, es subida de archivo (comportamiento actual)
    Logger.log("[DEBUG] No se encontró action válida, ejecutando uploadFile_");
    return uploadFile_(data);
    
  } catch (err) {
    Logger.log("[DEBUG] Error en doPost: " + String(err));
...
function getOrCreateFolder_(data) {
  try {
    Logger.log("[DEBUG] getOrCreateFolder_ llamado con: " + JSON.stringify(data));
    
    var folderName = data.folderName;
    var parentFolderId = data.parentFolderId;

    if (!folderName || !parentFolderId) {
      Logger.log("[DEBUG] Faltan folderName o parentFolderId");
      return json_(400, { ok: false, error: "Faltan folderName o parentFolderId" });
    }

    Logger.log("[DEBUG] Buscando carpeta: " + folderName + " en: " + parentFolderId);
    ...
    if (folders.hasNext()) {
      ...
      Logger.log("[DEBUG] Carpeta existente encontrada: " + existingFolder.getId());
      return json_(200, {
        ok: true,
    ...
    Logger.log("[DEBUG] Creando nueva carpeta: " + folderName);
    var newFolder = parentFolder.createFolder(folderName);
    Logger.log("[DEBUG] Carpeta creada: " + newFolder.getId());
    return json_(200, {
      ok: true,
      folderId: newFolder.getId(),
      webViewLink: "https://drive.google.com/drive/folders/" + newFolder.getId(),
    ...
    Logger.log("[DEBUG] Error en getOrCreateFolder_: " + String(err));
    return json_(500, { ok: false, error: String(err) });
  }
}
